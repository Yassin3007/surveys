                                            ...
                                            <td >
                                                ...
                                                @else
                                                    @if(\App\Models\StudentSurvey::query()->where('student_id',\App\Models\Student::query()->where('number',$student->trainee_number)->first()->id)
                                                        ...
                                                        <a href="{{route('student_survey_answer',['student_id'=>\App\Models\Student::query()->where('number',$student->trainee_number)->first()->id,'reference_number'=>$student->reference_number,'type'=>0,'survey_id'=>$subject_survey])}}">
                                            ...
                                            <td>
                                                ...
                                                @else
                                                    @if(\App\Models\StudentSurvey::query()->where('student_id',\App\Models\Student::query()->where('number',$student->trainee_number)->first()->id)
                                                        ...
                                                        <a href="{{route('student_survey_answer',['student_id'=>\App\Models\Student::query()->where('number',$student->trainee_number)->first()->id,'reference_number'=>$student->reference_number,'type'=>1,'survey_id'=>$teacher_survey])}}">
                                                            <span style="color: green;font-size: 30px" class="fa fa-check-circle"></span>
                                                            <br>
                                                            تم التقييم
                                                        </a>
